...
//added to allow veriables from other files***************************************************************************************************8
#include "button_push_count.h"
...
// change this to modify when the telemtry will be sent ******************************************************************************************************************

int azure_pnp_send_telemetry(azure_iot_t* azure_iot) {
  _az_PRECONDITION_NOT_NULL(azure_iot);

  if (button_push_count >= 2) {
    // Change the NeoPixel color to blue before sending telemetry
    strip.setPixelColor(0, strip.Color(0, 0, 255));
    strip.show();
    button_push_count = 0; // Reset the push count
  ...
  // Change the NeoPixel color back to green after sending telemetry
...
// This functions tells us what will be in the payload**************************************************************************************
  ...
  size_t* payload_buffer_length)
{
  ...
  // Append "event" property with the value "ButtonPushed"
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR("event"));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding 'event' property name to telemetry payload.");
  rc = az_json_writer_append_string(&jw, AZ_SPAN_FROM_STR("ButtonPushed"));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding 'ButtonPushed' property value to telemetry payload.");
